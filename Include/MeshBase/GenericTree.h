...
// -------------------------------------------------------------------
	//内部存储
struct AI3D_API quad_tree_node_data {
	std::string node_name;
	std::string file_name_obj;////L17.obj=L18_0  L18_1  L18_2 L18_3.obj
	//L22=L23_0+L23_1.....L23删除，弃用，

	std::string file_name_osgb;//L17.osgb

	void* my_mesh;//mesh信息
	int level=0;//层级

	double current_reserving_ratio = 1.0;//当前节点保留比例
	double rangelist_ratio = 1.0;//todo:一个遍历树节点赋值rangelist_ratio的接口

	int texture_pixel=512;//对应信息textinfo_map，例：512*512，或者1024*1024
};
template <typename T>
class AI3D_API TreeNode {
public:
	TreeNode* parentPtr;
...
public:
	...
	//int node_segmentation(std::string str, int segment_level);
	...
	TreeNode<T>* createRoot(const T& rootData) {
	...
	void deleteSubtree(TreeNode<T>* targetRoot) {
		...
		{
			TreeNode<T>* walkBack = targetRoot;
		...
		std::stack<TreeNode<T>*> nodesToExplore;
		...
		while (!nodesToExplore.empty()) {
			...
			TreeNode<T>* curNode = nodesToExplore.top();
...
// 			}
		...
		while (!nodesToDelete.empty()) {
			...
			TreeNode<T>* curNode = nodesToDelete.top();
...
// 			}
	...
	void compress() {
		...
		std::queue<TreeNode<T>*> nodesToExplore;
		...
		while (!nodesToExplore.empty()) {
			...
			TreeNode<T>* frontNode = nodesToExplore.front();
			nodesToExplore.pop();
			...
			std::vector<TreeNode<T>*> compressedChildrenPtrs;
...
// 	}
/*
	int node_segmentation(std::string input_file_name, int segment_level,double level_reserving_ratio) {
		...
		while (!node_to_process.empty()) {
			...
			double curr_reserving_ratio = curr_node.current_reserving_ratio;
			double new_reserving_ratio = curr_reserving_ratio * level_reserving_ratio;
			...
			for (auto it = res.begin(); it != res.end(); it++) {
				...
				node_inserted.current_reserving_ratio = new_reserving_ratio;
				if (level > 1)
	...
	*/
/*
	int node_remesh(std::string input_file_name, int segment_level) {
...
template class AI3D_API TreeNode<quad_tree_node_data>;//在实例化导出的时候前面一定要加上template关键字，否则会有错误 error C2906；
typedef   TreeNode<quad_tree_node_data>        QuadTreeNode;

template class AI3D_API GenericTree<quad_tree_node_data>;//在实例化导出的时候前面一定要加上template关键字，否则会有错误 error C2906；
//template class AI3D_API GenericTree<int>;

typedef   GenericTree<quad_tree_node_data>        QuadTree; 
//typedef   GenericTree<int>        QuadTree2; 

//AI3D_API GenericTree<quad_tree_node_data> tree;
